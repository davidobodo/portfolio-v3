...
However, after writing tests, how can we know if the amount of tests we have written are enough to cover all important places in our application ?

Say hello to `Jest "collect coverage"`.

With the "collect coverage" option provided by jest, we can easily know how much of our code has been tested and how much of our code is left to test.
...
That's not all, If you look at the root folder of your application, you would see that a "supposed hidden" folder named `coverage` has been created for you.
...
If you look at the table in the previous image, you would notice that the table has five columns:
...
The "red" highlighting is shows us, places of our code that were never tested. With this information, we know that we need to write tests for when our network request fails, (i.e line 39 in the image above) and for when a user tries to create a post and there is no text in the inputValue.
...
I hope you do use this knowledge to write robust tests that cover the entirety of your application as you want.
